...
t = et0plus(2 + unit * 4 + subunit).split(':'); 
v = (parseInt(t[0], 16) | ((subunit > 0) ? 2 : 0)) & 0xFF; 
t[0] = v.hex(2);
return t.join(':');
}
}
...
for (uidx = 1; uidx <= nvram.mwan_num; ++uidx){
u = (uidx > 1) ? uidx : '';
fom['wan'+u+'_mac'].value = E('_f_wan'+u+'_hwaddr').value; 
}
for (uidx = 0; uidx < wl_ifaces.length; ++uidx) {
u = wl_fface(uidx);
E('_wl'+u+'_hwaddr').value = E('_f_wl'+u+'_hwaddr').value; 
}
form.submit(fom, 1);
}
function init() {
