...
vifs_defined.push([
u.toString(),				
(nvram['wl'+u+'_ifname']) || ('wl'+u),	
u.substr(0, u.indexOf('.')),		
u.substr(u.indexOf('.')+1) || '-1',	
data[1] || '1',				
'0',					
data[1] || '1',				
data[3],				
data[2],				
(eval('nvram["wl'+u+'_hwaddr"]')) || mac_null, 
'0',					
data[4],
data[5]					
]);
...
vifs_defined[vif][4] = data[1]; 
vifs_defined[vif][8] = data[2]; 
vifs_defined[vif][7] = data[3]; 
vifs_defined[vif][11] = data[4]; 
vifs_defined[vif][12] = data[5]; 
...
var u = vifs_possible[vidx][0].toString();  
if (u.indexOf('.') > 0) { 
var vif = definedVIFidx(u);
if (vif >= 0) {
...
vif = definedVIFidx(u);
if (u == 0)
fom.wl_macmode.value = E('_f_wl'+u+'_macmode').value; 
if (vif < 0)
E('_wl'+u+'_macmode').name = '_f_wl'+u+'_macmode_'; 
else {
E('_wl'+u+'_macmode').value = E('_f_wl'+u+'_macmode').value;
E('_wl'+u+'_maclist').value = nvram['wl_maclist'].toString(); 
}
if (vif < 0) {
...
E('_wl'+u+'_nmcsidx').value = -2; 
E('_wl'+u+'_nbw').value = 0;
E('_wl'+u+'_bss_opmode_cap_reqd').value = 0; 
switch (E('_wl'+u+'_net_mode').value) {
case 'b-only':
...
E('_wl'+u+'_bss_opmode_cap_reqd').value = 1; 
break;
case 'bg-mixed':
break;
case 'a-only':
...
E('_wl'+u+'_bss_opmode_cap_reqd').value = 2; 
break;
case 'nac-mixed': 
E('_wl'+u+'_nmode').value = -1; 
E('_wl'+u+'_nmcsidx').value = -1; 
...
E('_wl'+u+'_nmode').value = -1; 
E('_wl'+u+'_nmcsidx').value = -1; 
E('_wl'+u+'_bss_opmode_cap_reqd').value = 3; 
break;
default: 
E('_wl'+u+'_nmode').value = -1;
E('_wl'+u+'_nmcsidx').value = -1;
...
fom._service.value = 'wlgui-restart'; 
}
...
nvram['wl'+u+'_radio'] || '0',	
wl_ifaces[uidx][6] || '0',	
nvram['wl'+u+'_bss_enabled'] || '1', 
wmode || 'disabled',		
wl_ifaces[uidx][4] || '',	
nvram['wl'+u+'_hwaddr'],	
wlvifs,				
bridged,
nvram['wl'+u+'_macmode'] || 'disabled' 
]);
